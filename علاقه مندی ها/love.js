...
const  btnc =document.querySelectorAll('#btn-c');
const prodjuct =document.querySelectorAll('#card-d');


hamberger2.addEventListener('click',Open);
hamberger.addEventListener('click',Close);

for(let i=0;i<=btnc.length;i++){/*                                   پاک کردن المنت   وقتی که کلیک شد با حلقه فور                   */
    btnc[i].addEventListener('click',m);
    function m(){
        prodjuct[i].style.display='none';
        if(i==btnc.length-1){/*------------------------                        وقتی که همه علافهمندی ها پاک شد بنویسه علاقمندی وجود ندارد                                  ------------------*/
            const note =document.createElement('h2');
            mainleft.appendChild(note);
            note.textContent='!علاقه مندی ایی وجود ندارد';
            note.style.color='red';
            note.style.textShadow='0 0 10px black'
            note.style.marginTop='300px';
            note.style.marginBottom='300px';
        }
    }
